...
@dataclass
class DRDataCollatorWithPadding:
    r"""
    Reward DataCollator class that pads the inputs to the maximum length of the batch.
    ...
    def __call__(self, features: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for feature in features:
            ...
            features_a_1.append(
                {
                    "input_ids": feature["input_ids_a_1"],
                    "attention_mask": feature["attention_mask_a_1"],
                }
            )
            features_a_2.append(
                {
                    "input_ids": feature["input_ids_a_2"],
                    "attention_mask": feature["attention_mask_a_2"],
                }
            )
            
            chosen_preference.append(feature["chosen_preference"])
            rejected_preference.append(feature["rejected_preference"])
            a_1_preference.append(feature["a_1_preference"])
            a_2_preference.append(feature["a_2_preference"])
        ...
        batch_a_1 = self.tokenizer.pad(
            features_a_1,
            padding=self.padding,
            pad_to_multiple_of=self.pad_to_multiple_of,
            return_tensors=self.return_tensors,
        )
        batch_a_2 = self.tokenizer.pad(
            features_a_2,
            padding=self.padding,
            pad_to_multiple_of=self.pad_to_multiple_of,
            return_tensors=self.return_tensors,
        ...
        batch = {
            ...
            "input_ids_rejected": batch_rejected["input_ids"],
            "attention_mask_rejected": batch_rejected["attention_mask"],
            "input_ids_a_1": batch_a_1["input_ids"],
            "attention_mask_a_1": batch_a_1["attention_mask"],
            "input_ids_a_2": batch_a_2["input_ids"],
            "attention_mask_a_2": batch_a_2["attention_mask"],
            'chosen_preference': torch.tensor(chosen_preference),
            'rejected_preference': torch.tensor(rejected_preference),
            'a_1_preference': torch.tensor(a_1_preference),
            'a_2_preference': torch.tensor(a_2_preference),
            "return_loss": True,
        }
        if has_margin:
            margin = torch.tensor(margin, dtype=torch.float)
